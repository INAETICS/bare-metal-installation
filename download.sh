...
echo "${GPG_KEY}" | gpg --batch --quiet --import

echo "Downloading the signature for: ${IMAGE_NAME}..."
wget -q -O "${SIG_NAME}" "${SIG_URL}"

echo "Downloading the actual image file: ${IMAGE_NAME}..."
wget -q -O "coreos_production_image.bin.bz2" "${IMAGE_URL}"

if ! gpg --trusted-key "${GPG_LONG_ID}" --verify "${SIG_NAME}"; then
